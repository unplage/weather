...
- 部署于 `/weather/` 子路径（`manifest.json` 的 `start_url: "/weather/"`），线上地址 `https://unplage.github.io/weather/`。
...
## 服务工作者（sw.js）
- `CACHE_NAME` 末尾版本号（当前 `v4`）在**每次修改应用代码时都必须递增**，否则浏览器喂旧缓存。这是最常见的坑。
- `BASE_PATH` 动态取自 `self.location.pathname`，适配任意子路径，勿硬编码。
- 缓存策略（仅拦截同源 GET）：导航 = 网络优先回退缓存；静态资源 + `cdn.jsdelivr.net` 图标字体 = 缓存优先；天气/地理 API（`.qweatherapi.com`、`.qweather.com`、`api.bigdatacloud.net`）= 缓存优先、后台更新。
